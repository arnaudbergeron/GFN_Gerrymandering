...
def get_trajectory_pfs(
    ...
    if valid_states.batch_shape != tuple(valid_actions.batch_shape):
        print(valid_states.batch_shape, valid_actions.batch_shape)
...
def get_trajectory_pbs(
    ...
    state_mask = (
        (~trajectories.states.is_sink_state & ~trajectories.states.is_initial_state) & trajectories.is_valid
    )
    action_mask = (~trajectories.actions.is_dummy & ~trajectories.actions.is_exit) & trajectories.is_valid[1:]
    valid_states = trajectories.states[state_mask]
    valid_actions = trajectories.actions[action_mask]
    if valid_states.batch_shape != tuple(valid_actions.batch_shape):
        print(valid_states.batch_shape, valid_actions.batch_shape)
        print(valid_states.tensor)
        raise AssertionError("Something wrong happening with log_pb evaluations")
    ...
    dist_county, dist_district = pb.to_probability_distribution(
        valid_states, estimator_outputs
    )

    log_probs_county = dist_county.log_prob(valid_actions.tensor[:,0].reshape(-1,1))
    log_probs_district = dist_district.log_prob(valid_actions.tensor[:,1].reshape(-1,1))

    valid_log_pb_actions = log_probs_county + log_probs_district
