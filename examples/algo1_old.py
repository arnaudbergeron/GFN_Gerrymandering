...
def accept_or_reject_proposal(current_partition, proposed_partition, G, BCP_current, V_CP, R, q, lambda_param, g_func,
    ...
    # Compute |B(CP, π)| -> Precomputed from the main loop BCP_current = |B(CP, π)|
...
def run_algorithm_1(df, reward_w, q, beta, num_samples, lambda_param=2, max_pop_dev_threshold=0.10,
    ...
    while i < num_samples:
        ...
        if proposed_partition != current_partition and accept_or_reject_proposal(current_partition,
            ...
            if proposed_reward > best_reward:
                ...
                print(
                    f"Iteration {i:03} | Avg Compact dev: {avg_compactness:.6f} | Curr Best: {best_reward:.6f} | New Best Reward: {proposed_reward:.6f} !")
            else:
                print(
                    f"Iteration {i:03} | Avg Compact dev:: {avg_compactness:.6f} | Curr Best: {best_reward:.6f} | Reward: {proposed_reward:.6f}")
...
def main(state_abrv="IA", seed=6162):
    ...
    Parameters:
    - state_abrv: The state abbreviation to load the data for.
    ...
    samples, best_partition = run_algorithm_1(df,  # DataFrame containing the nodes and their attributes
                                              ...
                                              q=0.04,  # 0.04 for PA, 0.05 for IA, proba of turning on edges
                                              beta=30,  # Inverse temp (higher beta => more emphasis on population)
                                              ...
                                              max_pop_dev_threshold=0.03,  # max pop deviation from equal districts
                                              compactness_threshold=0.27)  # [0, 1], lower is worse, cd_2020 is 0.33
    ...
    save_partitions_as_dataframe(all_partitions, output_file="output/mcmc_partitions.csv")

    ##############################################################
    # Best run: calculating metrics
